...
class MyThread(threading.Thread):
  def __init__(self,scriptName):
    ...
    lackey.wait(self.get_resource_path("images/lefttop.png"),999)
    lackey.wait(self.get_resource_path("images/righttop.png"),999)
    lackey.wait(self.get_resource_path("images/leftbottom.png"),999)
    self.lefttop = lackey.exists(self.get_resource_path("images/lefttop.png"))
    self.righttop1 = lackey.exists(self.get_resource_path("images/righttop.png"))
    self.leftbottom = lackey.exists(self.get_resource_path("images/leftbottom.png"))
    if not self.lefttop or not self.righttop1 or not self.leftbottom:
      self.show_error_message('未检测到游戏页面')
      return
    self.locationX = self.lefttop.x-50
    self.locationY = self.lefttop.y-50
    self.locationWidth = self.righttop1.x+self.righttop1.w-self.locationX+50
    self.locationHeight = self.leftbottom.y+self.leftbottom.h-self.locationY+50
    self.stoped = False
    self.clickBTime=0
  ...
  def outScript(self,current):
    ...
    with condition:
      if self.stoped == True:
        condition.wait()
    lackey.wait(current,999)
    lackey.wait(self.get_resource_path("images/xiulian.png"),999)
    locationOut = lackey.exists(self.get_resource_path("images/xiulian.png"))
    pyautogui.click(int(locationOut.x-20),int(
      locationOut.y-40))
    lackey.wait(self.get_resource_path("images/queding.png"),999)
    lackey.click(self.get_resource_path("images/queding.png"))
  # 整点
  def zhengDian(self):
     with condition:
      if self.stoped==True:
        condition.wait()
      print('打整点')
      lackey.wait(self.get_resource_path("images/opentalk.png"),999)
      lackey.click(self.get_resource_path("images/opentalk.png"))
  # 找图并且点击
  def findAndClickPic(self,A, B1,B2, C1,C2,D,E):
       ...
       with condition:
        if self.stoped == True:
          condition.wait()
       lackey.wait(A,999)
       while not lackey.exists(C1) and not lackey.exists(C2):
          with condition:
            if self.stoped == True:
              condition.wait()
          # 去除获得铜币黑框
          if lackey.exists(self.get_resource_path("images/huodetongbi.png")):
            lackey.click(self.get_resource_path("images/huodetongbi.png"))
          ...
          while E != '' and not EIsDown:
            ...
            if lackey.exists(B2):
              ...
              pyautogui.keyUp(E)
              break
          locationB1 = lackey.exists(B1)
          locationB2 = lackey.exists(B2)
          if locationB1:
              with condition:
                if self.stoped == True:
                  condition.wait()
              # print(locationB1,'locationB1')
              if E != '' and EIsDown:
                pyautogui.keyUp(E)
              lackey.click(B1)
              self.clickBTime = time.time()
          elif locationB2:
              # print(locationB2, 'locationB2')
              with condition:
                if self.stoped == True:
                  condition.wait()
              if E != '' and EIsDown:
  ...
  def guanduScript(self):
      ...
      with condition:
        if self.stoped == True:
          condition.wait()
      # 第一个河北军
      self.findAndClickPic(self.get_resource_path("images/caoyuanzhanchang.png"), self.get_resource_path("images/hbj3.png"), self.get_resource_path("images/hbj1.png"), self.get_resource_path("images/zdzd.png"),self.get_resource_path("images/zdzd1.png"),
                      '','')
      with condition:
        if self.stoped == True:
          condition.wait()
      ...
      with condition:
        if self.stoped == True:
          condition.wait()
      self.findAndClickPic(self.get_resource_path("images/hundian.png"), self.get_resource_path("images/wenchouzhihun.png"), self.get_resource_path("images/wenchouzhihun1.png"), self.get_resource_path("images/zdzd.png"),self.get_resource_path("images/zdzd1.png"), '', 'left')
      with condition:
        if self.stoped == True:
          condition.wait()
      # 魂殿进乌巢
...
class MyFrame(wx.Frame):
  def __init__(self):
    ...
    text.SetFont(font)
    # Start button
    self.button_start = wx.Button(self.panel, label="开始脚本(F5)", pos=(10, 80),style=wx.BORDER_NONE)
    ...
    # Pause button
    self.button_pause = wx.Button(self.panel, label="暂停脚本(F6)", pos=(146, 80),style=wx.BORDER_NONE)
    ...
    # Resume button
    self.button_resume = wx.Button(self.panel, label="继续脚本(F7)", pos=(10, 110),style=wx.BORDER_NONE)
