...
class GlobalStateMap:
    ...
    def __init__(self):
        ...
        self.dead_main_char_account_idx = None  # 需要被复活的主角账号索引
...
class CombatAutoScript:
    ...
    def handle_our_turn(self):
        ...
        # 重置我军辅助技能施法点位识别标志，以便下一轮非我方回合重新识别
    ...
    def _handle_single_main_char(self, dm_index):
        ...
        try:
            ...
            if not has_summon_button:
                ...
                # 但如果之前主角是存活的，应该检查一下是否真的死亡了
                if account.main_char.alive and not account.main_char.reviving:
                    # 尝试查找主角技能确认（短暂检查）
                    skill_name, skill_pos = self.find_skill_in_panel(
                        dm_index, self.main_char_skills, timeout=0.5
                    )
                    if not skill_name:
                        # 没有找到主角技能，且没有召唤按钮，说明主角可能死亡
                        with self._state_lock:
                            account.main_char.alive = False
                            self.state_map.dead_main_char_count += 1
                            self.state_map.dead_main_char_account_idx = dm_index  # 记录需要被复活的主角账号
                        self.report_battle_info(f"大漠对象{dm_index} 未检测到召唤按钮且未找到主角技能，标记主角为死亡", "warning")
                # 如果主角已经死亡，也需要记录账号索引（可能之前已经标记为死亡）
                elif not account.main_char.alive and not account.main_char.reviving:
                    with self._state_lock:
                        if self.state_map.dead_main_char_account_idx is None:
                            self.state_map.dead_main_char_account_idx = dm_index
                return
            ...
            if skill_name and skill_pos:
                ...
                with self._state_lock:
                    if account.main_char.alive and not account.main_char.reviving:
                        ...
                        # 优先使用当前账号的武将进行主角复活
                        # 等待武将面板出现（检查是否有存活的武将）
                        active_generals = account.get_active_generals()
                        has_alive_general = False
                        for gen_name, gen in active_generals:
                            if gen.alive:
                        ...
                        if has_alive_general:
                            # 有存活的武将，等待武将操作阶段进行复活
                            self.report_battle_info(f"大漠对象{dm_index} 主角死亡，等待武将操作阶段使用当前账号武将复活", "info")
                        elif len(active_generals) > 0:
                            # 已经识别过武将，但所有武将都死亡了，标记当前账号为全部阵亡
                            self.state_map.all_dead_account_indices.add(dm_index)
                            self.report_battle_info(f"大漠对象{dm_index} 主角和武将全部阵亡，标记为全部阵亡账号", "warning")
                        else:
                            # 还没有识别过武将（可能是第一回合或还未召唤），不标记为全部阵亡
                            # 等待后续回合可能召唤武将
                            self.report_battle_info(f"大漠对象{dm_index} 主角死亡，但尚未识别到武将，等待后续召唤", "info")
    ...
    def _handle_single_general(self, dm_index):
        ...
        try:
            ...
            # 等待操作按钮出现（确保技能面板已显示）
            if not self.wait_for_action_button(dm_index, timeout=1.0):
            ...
            # 1. 优先判断是否有需要复活的主角（必须在查找武将技能之前）
            if self.state_map.dead_main_char_count > 0:
                self.report_battle_info(f"大漠对象{dm_index} 检测到有主角死亡，优先尝试复活", "info")
                # 检查当前账号是否全部阵亡
                if dm_index in self.state_map.all_dead_account_indices:
                    # 当前账号全部阵亡，使用其他账号的单位进行复活
                    with self._state_lock:
                        revive_assigned = self.assign_revive_task_cross_account(dm_index, account)
                    if revive_assigned:
                        if self.try_revive_main_char(dm_index, account):
                            # 找到需要复活的主角账号，标记为复活中
                            dead_account_idx = self.state_map.dead_main_char_account_idx
                            if dead_account_idx is not None:
                                with self._state_lock:
                                    dead_account = self.state_map.get_account(dead_account_idx)
                                    dead_account.main_char.reviving = True
                                self.report_battle_info(f"大漠对象{dm_index} 使用其他账号单位复活账号{dead_account_idx}的主角", "action")
                            return
                        else:
                            # 跨账号复活失败，清除所有账号的复活任务分配，允许下一回合重新尝试
                            with self._state_lock:
                    ...
                    # 当前账号有存活单位，优先使用当前账号的武将进行复活
                    # 注意：此时已经等待过操作按钮了，不需要再次等待
                    with self._state_lock:
                        revive_assigned = self.assign_revive_task(account, dm_index)
                    if revive_assigned:
                        self.report_battle_info(f"大漠对象{dm_index} 已分配复活任务，开始尝试复活主角", "info")
                        if self.try_revive_main_char(dm_index, account):
                            # 复活操作完成，标记主角为复活中
                            dead_account_idx = self.state_map.dead_main_char_account_idx
                            if dead_account_idx is not None:
                                with self._state_lock:
                                    dead_account = self.state_map.get_account(dead_account_idx)
                                    dead_account.main_char.reviving = True
                                self.report_battle_info(f"大漠对象{dm_index} 使用当前账号武将复活账号{dead_account_idx}的主角", "action")
                            return
                        else:
                            # 同账号复活失败，清除当前账号的复活任务分配，允许下一回合重新尝试
                            with self._state_lock:
                                account.main_char.revive_assigned = None
                                account.general1.revive_assigned = None
                                account.general2.revive_assigned = None
                            self.report_battle_info(f"大漠对象{dm_index} 尝试复活主角失败，已清除当前账号的复活任务分配", "warning")
                    else:
                        self.report_battle_info(f"大漠对象{dm_index} 未能分配复活任务（可能主角已复活或正在复活中）", "info")
            ...
            # 如果前置判断找到了控制技能，且不需要清除状态，直接进入辅助武将逻辑
            if control_skill_found and control_skill_name and control_skill_pos and not has_enemies_need_clear:
                skill_name = control_skill_name
                skill_pos = control_skill_pos
                # 识别到辅助武将技能（前置判断找到的控制技能）
                self.report_battle_info(f"大漠对象{dm_index} 识别到辅助武将技能: {skill_name}（前置判断）", "success")
                with self._state_lock:
                    # 如果是第一回合且武将类型未设置，进行初始化
                    if self.is_first_turn:
                        # 分配给第一个未使用的武将位置
    ...
    def assign_revive_task(self, account, current_dm_index):
        """分配复活任务（当前账号内，按优先级：主角、辅助武将、攻击武将）
        只有当需要被复活的主角是当前账号的主角时，才分配当前账号的单位进行复活
        """
        if self.state_map.dead_main_char_count <= 0:
            return False
        ...
        # 检查需要被复活的主角是否已经在复活中
        dead_account_idx = self.state_map.dead_main_char_account_idx
        if dead_account_idx is None:
            # 如果没有记录需要被复活的主角账号，不分配任务
            return False
        
        # 关键修复：只有当需要被复活的主角是当前账号的主角时，才使用当前账号的单位进行复活
        if dead_account_idx != current_dm_index:
            # 需要被复活的主角是其他账号的，不应该使用当前账号的单位去复活
            return False
        ...
        if (account.main_char.revive_assigned or 
            account.general1.revive_assigned or 
            account.general2.revive_assigned):
            return False
        
        # 按优先级分配：主角 > 辅助武将 > 攻击武将（只检查实际存在的武将）
    ...
    def assign_revive_task_cross_account(self, current_dm_index, current_account):
        ...
        # 检查当前账号是否已经有单位被分配了复活任务
        if (current_account.main_char.revive_assigned or 
            current_account.general1.revive_assigned or 
            current_account.general2.revive_assigned):
                return False

        # 遍历所有账号，按优先级分配：主角 > 辅助武将 > 攻击武将
        for acc_idx in range(3):
            if acc_idx == current_dm_index:
                # 跳过当前账号（当前账号全部阵亡）
                continue
            ...
            if acc.main_char.alive and not acc.main_char.revive_assigned:
                acc.main_char.revive_assigned = True
                self.report_battle_info(f"分配账号{acc_idx}的主角进行跨账号复活", "info")
                return True
            ...
            for gen_name, gen in acc.get_active_generals():
                if gen.alive and gen.general_type == "support" and not gen.revive_assigned:
                    gen.revive_assigned = True
                    self.report_battle_info(f"分配账号{acc_idx}的辅助武将{gen_name}进行跨账号复活", "info")
                    return True
            ...
            for gen_name, gen in acc.get_active_generals():
                if gen.alive and gen.general_type == "attack" and not gen.revive_assigned:
                    gen.revive_assigned = True
                    self.report_battle_info(f"分配账号{acc_idx}的攻击武将{gen_name}进行跨账号复活", "info")
            return True
    ...
    def try_revive_main_char(self, dm_index, account):
        ...
        if target_pos:
            ...
            self.report_battle_info(f"大漠对象{dm_index} 使用复活药复活主角，找到目标图片位置: ({target_pos.x}, {target_pos.y})，施法位置: ({cast_x}, {cast_y})", "action")
            return True
        else:
            # 如果找不到目标图片，使用默认位置（根据被复活的主角账号索引）
            dead_account_idx = self.state_map.dead_main_char_account_idx
            if dead_account_idx is None:
                # 如果没有记录需要被复活的主角账号，使用账号0的默认位置
                dead_account_idx = 0
            
            # 根据账号索引使用不同的默认位置
            default_main_char_positions = {
                0: (793, 380),  # 大漠对象0主角默认位置
                1: (764, 278),  # 大漠对象1主角默认位置
                2: (825, 490),  # 大漠对象2主角默认位置
            }
            default_pos = default_main_char_positions.get(dead_account_idx, (793, 380))
            self.click_position(dm_index, default_pos[0], default_pos[1])
            time.sleep(CombatConstants.ACTION_DELAY)
            self.report_battle_info(f"大漠对象{dm_index} 使用复活药复活账号{dead_account_idx}的主角（未找到目标图片，使用默认位置: {default_pos}）", "warning")
            return True
    ...
    def update_global_state(self):
        ...
        # 如果主角复活成功，清除死亡标记
        if self.state_map.dead_main_char_account_idx is not None:
            dead_account_idx = self.state_map.dead_main_char_account_idx
            dead_account = self.state_map.get_account(dead_account_idx)
            if dead_account.main_char.alive and not dead_account.main_char.reviving:
                # 主角已复活，清除标记
                self.state_map.dead_main_char_account_idx = None
                if self.state_map.dead_main_char_count > 0:
                    self.state_map.dead_main_char_count -= 1
        
        # 检查全部阵亡的账号是否恢复（如果有单位存活，移除标记）
        for acc_idx in list(self.state_map.all_dead_account_indices):
            acc = self.state_map.get_account(acc_idx)
            if acc.main_char.alive:
    ...
    def handle_enemy_turn(self):
        ...
        if not self.enemy_target_detected_this_turn:
            ...
            if target_pos:
                # 调整坐标（根据实际游戏情况调整）
                target_pos.x = target_pos.x + 18
                target_pos.y = target_pos.y + 83
                self.state_map.enemy_target_position = (target_pos.x, target_pos.y)
                self.enemy_target_detected_this_turn = True
                self.report_battle_info(f"非我方回合，检测到攻击目标点位: {self.state_map.enemy_target_position}", "info")
            else:
                # 如果没找到，使用默认位置
                if not self.state_map.enemy_target_position:
                    self.state_map.enemy_target_position = self.default_enemy_target_position
                    self.report_battle_info(f"非我方回合，未检测到攻击目标，使用默认位置: {self.default_enemy_target_position}", "warning")
        
        # 2. 检测我军辅助技能施法点位（只识别一次，识别到后不再重复识别）
        if not self.ally_support_target_detected_this_turn:
            # 在我军区域内识别辅助技能施法点位（使用同样的 lantiao 图片）
            ...
            if ally_target_pos:
                ...
                self.report_battle_info(f"非我方回合，检测到我军辅助技能施法点位: {self.state_map.ally_support_target_position}", "info")
            else:
                # 如果没找到，使用默认位置
                if not self.state_map.ally_support_target_position:
                    self.state_map.ally_support_target_position = (764, 380)  # 默认我军中心位置
                    self.report_battle_info(f"非我方回合，未检测到我军辅助技能施法点位，使用默认位置: {self.state_map.ally_support_target_position}", "warning")
        
        # 3. 检测需要清除状态的敌军（只检测传入的敌军单位 key）
