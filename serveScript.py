...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.scriptName == "官渡":
            ...
            self._find_target_with_movement(
        ...
        # self.guajiAndzhengdianScript(f"{self.get_resource_path('serveAssets/images/guaji/bishuishuxue.bmp')}|{self.get_resource_path('serveAssets/images/guaji/bishuishuxue1.bmp')}")
            ...
            if self.zhengdianFloor not in ["全打", "走路", "龙+全打"]:
        ...
        try:
            ...
            if (
                ...
                if (
                    ...
                    return
                if (
                        self.zhengdianFloor in ["龙+全打", "全打", "牛+虎+兔"]
                        and self.scriptName in self.zhengdianFb
                ):
                    # 打整点
                    self.zhengdian_flag = True
                    self.outScript(A)
                    time.sleep(2)
                    if self.zhengdianFloor in ["全打", "龙+全打"]:
                        self.new_zhengdian()
                    else:
                        self.new_zhengdian1()
                    return
    ...
    def richangAndZhengDian(self):
        ...
        while not self.richangIsOver():
            ...
            if not result_item and not self.richangIsOver():
                print("打整点")
                if self.zhengdianFloor in ["龙+全打", "全打"]:
                    # 打整点
                    self.zhengdian_flag = True
                    time.sleep(2)
                    self.new_zhengdian()
...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        self.button_resume = wx.Button(
            self.panel, label="继续脚本(F7)", pos=(146, 65),
            style=wx.BORDER_NONE
        )
        ...
        self.button = wx.Button(
            self.panel, label="设置游戏信息", pos=(155, 5), size=(80, 25)
        )
        ...
        self.help_link = wx.StaticText(
            self.panel, label="说明", pos=(10, 235), style=wx.ST_NO_AUTORESIZE
        )
        ...
        self.updateVersion = wx.StaticText(
            self.panel, label="检查更新", pos=(190, 236),
            style=wx.ST_NO_AUTORESIZE
        )
        self.updateVersion.SetForegroundColour(wx.BLUE)
        ...
        self.userData = self.remote_info["userData"]
        self.mac_address = self.get_mac_address()
        ...
        self.dropdown = wx.ComboBox(
            self.panel, pos=(10, 5), size=(135, 30), choices=has_choices
        )
    ...
    def stop_script(self):
        ...
        if self.thread is not None and self.thread.is_alive():
            ...
            self.thread.join()
            if (
                    self.thread.child_thread is not None
                    and self.thread.child_thread.is_alive()
            ):
                self.thread.child_thread.join()
            if (
                    self.thread.win1_thread is not None
                    and self.thread.win1_thread.is_alive()
            ):
                self.thread.win1_thread.join()
            if (
                    self.thread.win2_thread is not None
                    and self.thread.win2_thread.is_alive()
            ):
...
class MyDialog(wx.Dialog):
    def __init__(self, parent, has_script):
        super().__init__(parent, title="设置游戏信息", size=(395, 300),
                         pos=(260, 30))

        panel = wx.Panel(self)
        self.team_leader_text = wx.TextCtrl(panel, pos=(10, 195),
                                            size=(100, 24))
        self.team_leader_text.SetHint("游戏名称")
        self.team_leader_pos = wx.ComboBox(
            panel, pos=(110, 195), size=(70, 24), choices=["1", "2", "3", "4"]
        )
        self.team_leader_pos.SetHint("方位")
        self.choiceZhanHunCeng = wx.ComboBox(
            panel, pos=(200, 195), size=(100, 24),
            choices=["26层", "27层", "手动27层"]
        )
        self.choiceZhanHunCeng.SetHint("炼狱层数")
        self.lianyu_count = wx.TextCtrl(
            panel, pos=(300, 195), size=(70, 24), validator=NumberValidator()
        )
        ...
        self.number_input = wx.TextCtrl(
            panel, pos=(110, 75), size=(70, 24), validator=NumberValidator()
        )
        self.number_input.SetHint("次数")
        self.qingyuan_count = wx.TextCtrl(
            panel, pos=(200, 75), size=(170, 24), validator=NumberValidator()
        )
        ...
        self.teammate1_pos = wx.ComboBox(
            panel, pos=(110, 165), size=(70, 24), choices=["1", "2", "3", "4"]
        )
        ...
        self.teammate2_pos = wx.ComboBox(
            panel, pos=(300, 165), size=(70, 24), choices=["1", "2", "3", "4"]
        )
        self.teammate2_pos.SetHint("方位")
        self.button = wx.Button(
            panel, label="确定", style=wx.BORDER_NONE, pos=(10, 225),
            size=(60, 24)
        )
        ...
        self.closeBloudBtn = wx.Button(
            panel, label="取消加血", style=wx.BORDER_NONE, pos=(310, 225),
            size=(60, 24)
        )
        ...
        self.zhanhun_count = wx.TextCtrl(
            panel, pos=(300, 105), size=(70, 24), validator=NumberValidator()
        )
        ...
        self.choiceZhengdian = wx.ComboBox(
            panel, pos=(10, 135), size=(170, 30), choices=choices
        )
...
class HelpDialog(wx.Dialog):
    def __init__(self, parent, title, content, images):
        super(HelpDialog, self).__init__(
            parent, title=title, size=(600, 400), pos=(260, 30)
        )
        ...
        for image_path in images:
            ...
            image = image.Scale(
                int(original_width / 2), int(original_height / 2),
                wx.IMAGE_QUALITY_HIGH
            )
            bitmap = wx.StaticBitmap(panel, -1, image.ConvertToBitmap())
            sizer.Add(bitmap, 0, wx.ALL | wx.LEFT, 0)
...
class UpdateDialog(wx.Dialog):
    def __init__(self, parent, remote_info):
        super().__init__(parent, title="检查更新", size=(500, 300),
                         pos=(260, 30))
    ...
    def InitUI(self):
        ...
        # 添加顶部信息区域
        top_info = wx.StaticBox(panel, label="版本信息")
        top_sizer = wx.StaticBoxSizer(top_info, wx.VERTICAL)

        grid = wx.FlexGridSizer(cols=2, vgap=10, hgap=20)
        grid.Add(wx.StaticText(panel, label="当前版本:"), 0,
                 wx.ALIGN_CENTER_VERTICAL)
        self.current_version_label = wx.StaticText(panel,
                                                   label=self.current_version)
        grid.Add(self.current_version_label, 0, wx.ALIGN_CENTER_VERTICAL)

        grid.Add(wx.StaticText(panel, label="最新版本:"), 0,
                 wx.ALIGN_CENTER_VERTICAL)
        self.latest_version_label = wx.StaticText(panel, label="检查中...")
        grid.Add(self.latest_version_label, 0, wx.ALIGN_CENTER_VERTICAL)

        top_sizer.Add(grid, 0, wx.EXPAND | wx.ALL, 10)

        # 添加更新日志区域
        changelog_box = wx.StaticBox(panel, label="更新日志")
        changelog_sizer = wx.StaticBoxSizer(changelog_box, wx.VERTICAL)

        self.changelog_text = wx.TextCtrl(panel,
                                          style=wx.TE_MULTILINE | wx.TE_READONLY)
        changelog_sizer.Add(self.changelog_text, 1, wx.EXPAND | wx.ALL, 4)
        self.changelog_text.Disable()
        # 添加按钮区域
        button_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.update_button = wx.Button(panel, label="立即更新")
        ...
        vbox.Add(changelog_sizer, 1, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM,
                 10)
        vbox.Add(button_sizer, 0, wx.ALIGN_RIGHT | wx.RIGHT | wx.BOTTOM, 10)
