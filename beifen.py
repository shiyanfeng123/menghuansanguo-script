...
class MyThread(threading.Thread):
	...
	def feiZhengDian(self, fei_image, base_image, scroll_deg):
		try:
			...
			shengxiaoLocation = pyautogui.locateOnScreen(
				fei_image,
				confidence=self.confidenceNum,
				region=(
					self.locationX,
			...
			while not pyautogui.locateOnScreen(
				...
				feiLocation = pyautogui.locateCenterOnScreen(
					self.get_resource_path("images/fei.png"),
					confidence=self.confidenceNum,
					region=(
						shengxiaoLocation.left - 100,
						shengxiaoLocation.top,
						shengxiaoLocation.left + 300,
						shengxiaoLocation.top + 45,
					),
				)
				if feiLocation:
	...
	def findAndClickPic(self, A, B1, B2, C1, C2, D, E, E2=None, E2DownTime=1):
		...
		while not pyautogui.locateOnScreen(
		...
		) and not pyautogui.locateOnScreen(
			C2,
			confidence=self.confidenceNum,
			region=(
					self.locationX,
			...
			# self.click_nth_image(self.get_resource_path("images/addBloud.png"), (self.locationX, self.locationY, int(self.locationWidth * 0.5), int(self.locationHeight * 0.5)), 3)
			if self.scriptName == "官渡" and time.time() - startTime > 20:
