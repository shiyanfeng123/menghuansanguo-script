...
class MyThread(threading.Thread):
	...
	def run(self):
		...
		if mac_address in self.userInfoMac:
			...
			return
		self.autoClick.get_winds(self.frame.game_name)
	...
	def child_task(self):
		while True:
			...
			# )
			# 点击取消
			if pyautogui.locateOnScreen(self.get_resource_path("images/jingji.png"), confidence=self.confidenceNum, region=self.gameLocation):
				self.click_image(
...
class MyFrame(wx.Frame):
	...
	def on_button_click(self, event):
		...
		if dialog.ShowModal() == wx.ID_OK:
			...
			print(self.teammate1_name)
			print(self.teammate2_name)
...
class AutoClick():
	...
	__PostMessageW = win32api.PostMessage
	__SendMessageW = win32api.SendMessage
	...
	def find_image_in_region(self, screen, template, region):
		"""
		在指定区域内查找图像

		:param screen: 全屏截图，numpy数组
		:param template: 要查找的模板图像，numpy数组
		:param region: 查找区域，格式为 (left, top, width, height)
		:return: 匹配结果，包含坐标和置信度
		"""
		left, top, width, height = region
		# 裁剪出指定区域
		cropped_screen = screen[top:top + height, left:left + width]
		# 进行图像匹配
		result = ac.find_template(cropped_screen, template, 0.8)
		if result:
			# 调整坐标到原图中的位置
			result['result'] = (result['result'][0] + left, result['result'][1] + top)
		return result
